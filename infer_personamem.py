...
class Evaluation:
    ...
    def query_llm(self, question, all_options, context=None, instructions=None):
        ...
        if instructions is None:
            ...
            instructions = (
                "你是一个根据用户属性树，从候选选项中选择最优回答的决策模块。\n\n"
                "你将看到：\n"
                "1. 用户问题；\n"
                "2. 若干候选回答选项；\n"
                "3. 该用户的属性树。\n\n"
                "你的目标是：**充分利用用户属性树中的信息，为这个用户的问题选出最合适的一条选项回答**。\n\n"
                "请严格遵循以下规则进行选择：\n"
                "- 明确使用用户属性树中的信息来对比各个选项的适配度；\n"
                "- 如果多个选项在通用场景下都合理，优先选择**最符合该用户属性树和个性化需求**的选项；\n"
...
def run_evaluation(cmd_args, llm):
    ...
    for row_data in tqdm(load_rows_with_context(question_path), total=total_rows):
        ...
        try:
            ...
            context = json.dumps(processed, indent=4, ensure_ascii=False).strip()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--result_path', type=str, default='results/eval_results_32k.csv', help='Path to save the results CSV file')
...
# nohup python -u infer_personamem.py  > results/eval_results_32k.log 2>&1 &
